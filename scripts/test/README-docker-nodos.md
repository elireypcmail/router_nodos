...
3. `envs/tienda-N.env` con `NODO_ID` / `NODO_API_TOKEN` tras provisioning en hub-ui.
   ...
   **Hub en el Mac (no en Docker):** dentro del contenedor API usa
   `ROUTER_EVENTS_URL=http://host.docker.internal:3000` (no `localhost:3000`).

   Cambiar env: editar `tienda-N.env` y recrear el servicio:
   `docker compose -f scripts/test/docker-compose.nodos.yml up -d --no-build --force-recreate nodo-tienda-N`
...
## Zona horaria (`outbox_enqueued_at`)

Los triggers MySQL guardan `NOW(3)` como hora **local de tienda** (naive). El nodo convierte a UTC (`NODO_STORE_TZ=America/Caracas`) y el portal muestra en la zona del navegador.

Si MySQL corre en **UTC** (contenedor sin TZ), el timestamp naive queda adelantado ~4 h y en el portal verás fechas como **4/7 2:04 a. m.** en lugar de **3/7 10:04 p. m.** El compose fija `--default-time-zone=-04:00` y `TZ=America/Caracas` en MySQL.

Ventas ya ingestadas con el desfase conservan el UTC incorrecto en el hub; haz una venta nueva tras recrear MySQL o re-push transaccional para comprobar.
